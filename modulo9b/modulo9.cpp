...
double **alocaMatrizD(int a, int l) {
	int i;
	double **data = (double **) malloc (sizeof(double *) * a);
	for(i=0; i<a; i++) {
		data[i] = (double *) malloc (sizeof(double) * l);
	}
...
double gauss(int x, int y, double s) {
       double g = 1/(2 * PI * (s*s)),
              e = exp(-(x*x + y*y)/(2* s*s));
...
double ** mascara(int altura, int largura, double s){
       double **masc = alocaMatrizD(altura, largura);
       
       int i,j;
       
       for(i=0; i<altura; i++){
                for(j=0; j<largura; j++){
                         masc[i][j] = gauss(i,j,s);
                }
       }
